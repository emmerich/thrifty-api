...
import thrifty.api.parser.ItemParser;
import thrifty.api.parser.Parseable;

@Component
public class WikiaTextItemParser implements ItemParser {

    @Autowired
    private WikiaTextStatParser statParser;
	...
	public static void main(String[] args) {
		...
		WikiaTextItemParser parser = new WikiaTextItemParser();
		
		Item item = parser.toItem(text);
		
		System.out.println(item.name());
		System.out.println("Stats: " + item.statistics());
	}
	...
	public Item toItem(Parseable parseableItem) {
        ...
        item.name(parseableItem.getPageName());
        
        // "regular" stats are ones that are not part of a unique passive, or anything like that
        Set<Statistic> regularStatistics = getStatistics(getStats(infoBox, "effects"));
        item.addStatistic(regularStatistics);

        return item;
	}
	
	public Set<Statistic> getStatistics(Map<String, Integer> stats) {
		Set<Statistic> result = new HashSet<Statistic>();
		
		for(String stat : stats.keySet()) {
			result.add(new Statistic(statParser.toStat(stat), stats.get(stat)));
		}
